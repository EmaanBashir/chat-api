...
class ThreadManager(models.Manager):
    ...
    def create_group_thread(self, thread_name, *args):
        thread = self.create(thread_type = 'group', name = thread_name)
        for user in args:
            thread.users.add(user)
        return thread

class MessageManager(models.Manager):
    def mark_as_read(self, thread, user):
        unread_messages = thread.message_set.exclude(read_by__in = [user])
